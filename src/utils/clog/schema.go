...
type Schema interface {
	...
	SetResource(resource types.Resource) *LogContext
...
func (s *schema) SetResource(resource types.Resource) *LogContext {
	return getLogContext(s).setDepth(2).SetResource(resource)
}
...
const (
	...
	LF_Pin                  = "pin"
	LF_CustomFileLine       = "custom_file_line"
	LF_FuncName             = "func_name"
	LF_ResponseCode         = "response_code"
	LF_ResponseMsg          = "response_msg"
	LF_WorkflowId           = "workflowId"
)

// base log : use function and interface for mock
var Blog = func() Schema {
	return &schema{
		fields: []*field{
			// basic
			&field{key: LF_Schema, value: "JB"}, // jvessel base schema
...
// event log
var Elog = func() Schema {
	return &schema{
		fields: []*field{
			// basic
			&field{key: LF_Schema, value: "JE"}, // jvessel event schema
			&field{key: LF_Version, value: "v1"},
			&field{key: LF_ProcName},
			&field{key: LF_Host},
			&field{key: LF_Timestamp},
...
// http log
var Hlog = func() Schema {
	return &schema{
		fields: []*field{
			// basic
			&field{key: LF_Schema, value: "JH"}, // jvessel event schema
			&field{key: LF_Version, value: "v1"},
			&field{key: LF_ProcName},
			&field{key: LF_Host},
			&field{key: LF_Timestamp},
...
// audit log
var Auditlog = func() Schema {
	return &schema{
		fields: []*field{
			// basic
			&field{key: LF_Schema, value: "JA"}, // jvessel event schema
			&field{key: LF_Version, value: "v1"},
			&field{key: LF_Host},
			&field{key: LF_Timestamp},
			&field{key: LF_Level},
			&field{key: LF_CustomFileLine},
			&field{key: LF_FuncName},
			...
			&field{key: LF_Method},
			&field{key: LF_Path},
			&field{key: LF_ResponseCode},
			&field{key: LF_ResponseMsg},
		},
	}
